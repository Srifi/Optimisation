...
Points_support=find(alph0>epsi)' % points supports = contraintes actives...alph>0
...
for i=1:npx
    for j=1:npy
        ps = prodw(X,lab,alph0,[xp(i),yp(j)]'); % calcul de <w,x> + b sur une grille,on n'a pas besoin explicitement de w 
        V(i,j) = ps + b; 
    end
end
hold on
